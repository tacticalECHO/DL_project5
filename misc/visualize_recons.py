...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Visualize reconstruction-based inpainting results.")
    ...
    parser.add_argument("--mask-path", type=str, default="data/fixed_masks_medium.pt")
    parser.add_argument("--image-size", type=int, default=256)
    parser.add_argument("--batch-size", type=int, default=8)
    parser.add_argument("--num-workers", type=int, default=0)
    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--num-samples", type=int, default=8)
    parser.add_argument("--output", type=str, default="outputs/rcon/vis_medium.png")
...
def main() -> None:
    ...
    samples = collect_samples(model, loader, device, args.num_samples)
    title = args.title or f"Reconstruction Inpainting Visualization | {Path(args.mask_path).name}"
